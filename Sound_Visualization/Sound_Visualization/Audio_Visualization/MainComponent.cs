...
namespace Sound_Visualization
{
    public class MainComponent : GH_Component
    {
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            pManager.AddIntegerParameter("Columns", "c", "Number of columns", GH_ParamAccess.item);

            pManager.AddIntegerParameter("Rows", "r", "Number of rows", GH_ParamAccess.item);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            int columns = 0;
            DA.GetData("Columns", ref columns);

            int rows = 0;
            DA.GetData("Rows", ref rows);
            ...
            {
                case 1:
                    WaveVisualization geometries = new WaveVisualization(srf1, intensity, volume, columns, rows);
                ...
                case 2:
                    //Wavy surface visualization
                    //
                    break;

                case 3:
                    //Wavy mesh visualization
                    break;

                case 4:
                    //Perlin noise surface
                    PerlinNoiseVisualization perlin = new PerlinNoiseVisualization(srf1, intensity, volume, columns, rows);

                    boxes = perlin.GetBoxes();
                    List<int> Height = new List<int>();
                    foreach (Box b in boxes)
                    {
                        var height = Math.Round(b.Center.Z);
                        var intHeight = Convert.ToInt32(height);
                        Height.Add(intHeight);
                    }
                    DA.SetData("VolumeOut", volAfter);
                    DA.SetDataList("GeometryVisuals", boxes);
                    //DA.SetDataList("c", colors2);
                    DA.SetDataList("Tester", Height);
                    //DA.SetDataList("Tester2", h2);
